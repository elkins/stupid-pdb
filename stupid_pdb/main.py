...
def main():
    ...
    logger.debug(
        "Parsed arguments: length=%s, output='%s', full_atom=%s, sequence='%s', plausible_frequencies=%s",
        args.length, args.output, args.full_atom, args.sequence, args.plausible_frequencies
    )
    ...
    try:
        ...
        if args.output:
            ...
            if args.sequence:
                ...
                sequence_tag = args.sequence.replace('-', '')[:10] # Take first 10 chars, remove hyphens
