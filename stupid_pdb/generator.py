...
from .data import STANDARD_AMINO_ACIDS, ONE_TO_THREE_LETTER_CODE, AMINO_ACID_ATOMS, AMINO_ACID_FREQUENCIES, BOND_LENGTH_N_CA, BOND_LENGTH_CA_C, BOND_LENGTH_C_O, ANGLE_N_CA_C, ANGLE_CA_C_N, ANGLE_CA_C_O
...
def _resolve_sequence(length: int, user_sequence_str: str = None, use_plausible_frequencies: bool = False) -> list[str]:
    ...
    if user_sequence_str:
        ...
        if '-' in user_sequence_str_upper:
            ...
            return amino_acids
        elif len(user_sequence_str_upper) == 3 and user_sequence_str_upper in STANDARD_AMINO_ACIDS:
...
def generate_pdb_content(length: int = None, full_atom: bool = False, sequence_str: str = None, use_plausible_frequencies: bool = False) -> str:
    ...
    if not sequence:
        ...
        raise ValueError("Length must be a positive integer when no sequence is provided and no valid sequence string is given.")
    ...
    for i, aa_3l_code in enumerate(sequence):
        ...
        if not full_atom:
            ...
            o_x_offset_relative_to_C = BOND_LENGTH_C_O * np.cos(np.pi - ANGLE_CA_C_O_RAD)
            o_y_offset_relative_to_C = BOND_LENGTH_C_O * np.sin(np.pi - ANGLE_CA_C_O_RAD)
            o_coords = c_coords + np.array([o_x_offset_relative_to_C, o_y_offset_relative_to_C, 0.0])
            ...
            pdb_lines.append(PDB_ATOM_FORMAT.format(
                atom_number=atom_count, atom_name='CA', alt_loc='', residue_name=aa_3l_code,
                chain_id='A', residue_number=residue_number, insertion_code='',
                x_coord=ca_coords[0], y_coord=ca_coords[1], z_coord=ca_coords[2], 
                occupancy=1.00, temp_factor=0.00, element='C', charge=''
            ))
            atom_count += 1
